...
def load_cached(cache_dir, path: str, filename: str, min_length: int, max_length: int, n_samples: int):
    """"
        Read from cache if available. Used for certain pile sources and xsum
        to ensure fairness in comparison across attacks.runs.
    """
    file_path = os.path.join(cache_dir, f"cache_{min_length}_{max_length}_{n_samples}", path, filename + ".jsonl")
...
def dump_to_cache(data: List[str], cache_dir, path, filename: str, min_length: int, max_length: int, n_samples: int):
    """
        Cache a file (one text sequence per line)
    """
    # Make sure path directory exists
    subdir = os.path.join(cache_dir, f"cache_{min_length}_{max_length}_{n_samples}", path)
